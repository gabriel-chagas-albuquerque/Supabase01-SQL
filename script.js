const SUPABASE_URL = "https://ljaaunahoavbahkjarxv.supabase.co";
const SUPABASE_ANON_KEY ="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxqYWF1bmFob2F2YmFoa2phcnh2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDg2NDkwMDUsImV4cCI6MjA2NDIyNTAwNX0.3PtoIAAGBBvZqvl5_Bh1S7Gh0gr3UobM-7RSawyP2ks";
...
async function fetchTodos() {
  if (
    ...
    if (loadingMessage)
      loadingMessage.textContent = "Supabase não configurado corretamente.";
    if (emptyMessage) emptyMessage.classList.add("hidden");
    if (SUPABASE_URL === "SUA_SUPABASE_URL" || SUPABASE_URL === "") {
      showMessage(
        "Por favor, configure suas credenciais do Supabase no script JavaScript!",
        "error",
        7000
      );
    }
  ...
  try {
    ...
    showMessage(
      `Erro ao buscar tarefas: ${err.message || "Erro desconhecido"}`,
      "error"
    );
    if (loadingMessage)
      loadingMessage.textContent = "Erro ao carregar tarefas.";
  } finally {
    if (loadingMessage) loadingMessage.classList.add("hidden");
  }
}

function renderTodoItem(todo, prepend = true) {
  // Adicionado parâmetro prepend
...
async function addSingleTaskToDbAndUi(taskText, fromSuggestion = false) {
  ...
  if (
    ...
    showMessage(
      "Supabase não configurado. Não é possível adicionar tarefa.",
      "error"
    );
    return null;
  }
  ...
  try {
    ...
    showMessage(
      `Erro ao adicionar tarefa: ${err.message || "Erro desconhecido"}`,
      "error"
    );
    return null;
  }
...
async function toggleComplete(id, currentDbState) {
  // currentDbState é o estado ANTES do clique
  if (
    !supabaseClient ||
    SUPABASE_URL === "SUA_SUPABASE_URL" ||
    SUPABASE_URL === ""
    ...
    showMessage(
      "Supabase não configurado. Não é possível atualizar tarefa.",
      "error"
    );
    return;
  }
  ...
  try {
    ...
    showMessage(
      `Erro ao atualizar tarefa: ${err.message || "Erro desconhecido"}`,
      "error"
    );
...
async function deleteTodo(id) {
  if (
    ...
    showMessage(
      "Supabase não configurado. Não é possível excluir tarefa.",
      "error"
    );
    return;
  }
  try {
    ...
    showMessage(
      `Erro ao excluir tarefa: ${err.message || "Erro desconhecido"}`,
      "error"
    );
  }
}
...
async function getSubtaskSuggestions(mainTaskText) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `API Gemini respondeu com status ${response.status}: ${
    ...
    if (
      ...
      console.warn(
        "Resposta da API Gemini não contém sugestões válidas:",
        result
      );
...
if (suggestSubtasksBtn) {
  suggestSubtasksBtn.addEventListener("click", async () => {
    ...
    if (suggestedSubtasks && suggestedSubtasks.length > 0) {
      showMessage(
        `IA sugeriu ${suggestedSubtasks.length} subtarefas! Adicionando...`,
        "success",
        2000
      );
